...
public class MasterShaderEditor : MaterialEditor
{
    ...
    public override void OnEnable()
    {
        ...
        string[] keywordsCurrent = material.shaderKeywords;
        ...
        frontMode = frontGradientOn
            ? ColorMode.GRADIENT
            : (frontSolidOn ? ColorMode.SOLID : ColorMode.OFF);
        ...
        backMode = backGradientOn
            ? ColorMode.GRADIENT
            : (backSolidOn ? ColorMode.SOLID : ColorMode.OFF);
        ...
        leftMode = leftGradientOn
            ? ColorMode.GRADIENT
            : (leftSolidOn ? ColorMode.SOLID : ColorMode.OFF);
        ...
        rightMode = rightGradientOn
            ? ColorMode.GRADIENT
            : (rightSolidOn ? ColorMode.SOLID : ColorMode.OFF);
        ...
        if (!ArrayUtility.Contains(material.shaderKeywords, "COLR_1_2") &&
            !ArrayUtility.Contains(material.shaderKeywords, "WORLD_SPACE_ON"))
        {
            material.shader = Shader.Find("Colr/Master Shader Local Space");
            Repaint();
        }
    ...
    public override void OnInspectorGUI()
    {
        ...
        if (material.shader.name != "Colr/Master Shader")
        {
            Repaint();
        ...
        if (isVisible && !shdr.hasMultipleDifferentValues && shdr.objectReferenceValue != null)
        {
            ...
            using (var group = new EditorGUILayout.FadeGroupScope(showFront.faded))
            {
                if (group.visible)
                {
                    ...
                    {
                        ColorProperty(GetMaterialProperty(targets, "_FrontTopColor"),
                            "Front Top Color");
                        ColorProperty(GetMaterialProperty(targets, "_FrontBottomColor"),
                            "Front Bottom Color");
                    }
                    else
                    {
            ...
            using (var group = new EditorGUILayout.FadeGroupScope(showBack.faded))
            {
                if (group.visible)
                {
                    ...
                    {
                        ColorProperty(GetMaterialProperty(targets, "_BackTopColor"),
                            "Back Top Color");
                        ColorProperty(GetMaterialProperty(targets, "_BackBottomColor"),
                            "Back Bottom Color");
                    }
                    else
                    {
            ...
            using (var group = new EditorGUILayout.FadeGroupScope(showLeft.faded))
            {
                if (group.visible)
                {
                    ...
                    {
                        ColorProperty(GetMaterialProperty(targets, "_LeftTopColor"),
                            "Left Top Color");
                        ColorProperty(GetMaterialProperty(targets, "_LeftBottomColor"),
                            "Left Bottom Color");
                    }
                    else
                    {
            ...
            using (var group = new EditorGUILayout.FadeGroupScope(showRight.faded))
            {
                if (group.visible)
                {
                    ...
                    {
                        ColorProperty(GetMaterialProperty(targets, "_RightTopColor"),
                            "Right Top Color");
                        ColorProperty(GetMaterialProperty(targets, "_RightBottomColor"),
                            "Right Bottom Color");
                    }
                    else
                    {
            ...
            {
                independentSides =
                    EditorGUILayout.Toggle("Do not mix colors of sides", independentSides);
                EditorGUILayout.Separator();
            }
            ...
            if (frontMode != ColorMode.SOLID || backMode != ColorMode.SOLID ||
                ...
                fogBottom)
            {
                FloatProperty(GetMaterialProperty(targets, "_GradientYStartPos"),
                    "Gradient start Y");
                float h = FloatProperty(GetMaterialProperty(targets, "_GradientHeight"),
                    "Gradient height");
                if (h < 0)
                {
                    material.SetFloat("_GradientHeight", 0);
                }
    ...
    private void ApplyProperties()
    {
        ...
        if (!fogBottom && material.IsKeywordEnabled(FOG_BOTTOM))
        {
        ...
        foreach (var keyword in keywordsUpdated)
        {
